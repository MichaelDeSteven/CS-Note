...
![](https://raw.githubusercontent.com/MichaelDeSteven/CS-Note/master/%E8%AE%A1%E7%AE%97%E6%9C%BA%E5%9F%BA%E7%A1%80%26%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F/pic/CPU基本组成原理图.png)
...
![](https://raw.githubusercontent.com/MichaelDeSteven/CS-Note/master/%E8%AE%A1%E7%AE%97%E6%9C%BA%E5%9F%BA%E7%A1%80%26%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F/pic/全相联.png)
...
![](https://raw.githubusercontent.com/MichaelDeSteven/CS-Note/master/%E8%AE%A1%E7%AE%97%E6%9C%BA%E5%9F%BA%E7%A1%80%26%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F/pic/组相联映射.png)
...
* 虚拟存储器具备调入功能和置换功能，能从逻辑上对内存容量进行扩充的一种存储器系统
...
![](https://raw.githubusercontent.com/MichaelDeSteven/CS-Note/master/%E8%AE%A1%E7%AE%97%E6%9C%BA%E5%9F%BA%E7%A1%80%26%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F/pic/页表项数据结构.png)
...
![](https://raw.githubusercontent.com/MichaelDeSteven/CS-Note/master/%E8%AE%A1%E7%AE%97%E6%9C%BA%E5%9F%BA%E7%A1%80%26%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F/pic/请求分页地址变换过程.png)
...
* 改进的CLOCK算法：考虑到页修改写回外存的情况，未被修改的页置换时并不需要写回外存，而修改的页则必须写入外存这样的性能开销会变大，因此该算法除了考虑访问位A之外还考虑了修改位M，算法步骤是，从指针指示的当前位置开始，扫描循环队列，优先寻找（A=0，M=0）的页面，将其作为淘汰页，同时第一次扫描时并不改变A，如果第一轮为找到理想淘汰页，则开始第二轮寻找（A=1，M=0）的页面，将其作为淘汰页，并将所有扫描的页面A置为0，如果第二轮失败则重复第一轮的步骤，如果仍失败，再重复第二轮的步骤，四个步骤之后，就一定能够找到淘汰页，该算法减少了磁盘IO的次数
...
* 特权指令是指在系统态运行的指令，它对内存空间的访问范围基本不受限制，同时能够启动外部设备、设置系统时钟时间、关中短、转换执行状态等，特权指令只允许OS使用，不允许用户进程使用，避免引起系统混乱
...
系统程序本地就是应用程序请求内核完成某功能时的一种过程调用，它是一种特殊的过程调用，首先它运行在系统态，而一般的过程调用都是在用户态完成的，运行系统调用时，由于调用和被调用过程是工作在不同的系统状态的，所以需要先由用户态转换为系统态，待内核执行完毕后，才从系统态切回用户态
...
除了上述三类系统调用外，常见的系统调用还有设备管理类（申请设备、释放设备、设备IO、重定向）信息维护类（获取或更新系统的信息）
...
1. 什么是程序直接控制I/O方式？说明其工作原理。
   ...
   A：I/O设备将自己的状态放入状态寄存器，OS不断地查询状态寄存器中的特定状态，直到I/O设备准备就绪，就进行读写操作
...
2. 什么是中断控制I/O方式？ 说明其工作原理。
   ...
   A：CPU发出控制l命令启动I/O后，转去执行其他的工作，当外设就绪时，向CPU发送中断请求，CPU响应中断请求，调用中断服务系统，执行数据传送，然后返回断点执行

3. 什么是DMA控制I/O方式？ 说明其工作原理。

   A：开始时，DMA发送“启动DMA传送”，发送完命令后，CPU转去执行其他任务，而请求I/O的用户进程会进入阻塞状态，DMA控制器控制外设和主存进行数据交换，完成I/O操作后，发起“DMA完成”请求中断信号，CPU检测到信号后就暂停当前执行的任务，转去执行请求I/O的用户进程
...
* 相对于BIO，NIO在执行系统调用时会返回错误码，使得程序可以继续运行，在数据传输到内核缓冲区时，进程会不断进行系统调用查询数据是否都准备好，如果没有传输完毕则返回错误，否则说明所有数据已经到达内核缓存区，然后就可以将内核缓冲区的数据拷贝到进程缓冲区当中
...
* 也叫做事件驱动IO，用户调用·select进程将会阻塞，然后内核会检查每一个select所负责的socket，如果某一个socket数据准备好了，那么select就会返回，此时进程就能够进行内核缓冲区到进程缓冲区的数据拷贝
...
* 中断和异常：中断可以随时产生，并不需要考虑处理器时钟同步，异常需要考虑处理器时钟同步

**中断响应过程**

* 保护断点和程序状态：为了保证中断处理完成后能够继续执行程序，需要保存现场，即把下一条要执行的指令，以及当前程序的数据和指令保存到寄存器当中
* 关中断：在执行中断程序时又出现了新的中断，开/关中断机制就是用来开启/禁止处理中断时再去响应新的中断，一般通过中断允许位来实现，中断允许位为1则为开中断，0表示关中断
...
由于中断服务程序的执行并不存在于进程上下文，因此要求中断服务程序的执行并不存在于进程上下文，因此要求中断服务程序的时间尽可能的短，为了在中断执行时间尽可能短和中断处理完成需要大量耗时工作找到一个平衡点，Linux将中断处理分成了两个部分：上半部和下半部
...
在linux设备驱动中，使用中断的设备需要申请和释放相应的request_irq()和free_irq()函数
