...
思路：第一遍dfs找距离根节点最远的结点u（一定是叶子结点），第二遍从叶子结点u开始dfs找距离结点u的最远结点v（一定是叶子结点），则u到v的路径一定是树的直径
...
思路：以当前结点为根节点，从孩子结点找到最长路径d1和次最长路径d2，则经过该根节点的最长路径一定为d1+d2，向其父结点返回d1，dfs完毕后遍历每个结点，寻找以每个结点为根节点的最长路径，最大值即为树的直径，定义$f(u)$为以u为根节点的最长路径，则状态转移方程有
$$
...
```cpp
// dep[i]表示结点i到根节点的深度, fa[i][k]表示结点i向上走2^k步到达的祖先节点
void bfs() {
...
以某件物品i为根节点，选择该物品并选择了该物品的若干个孩子结点，使得所有物品总体积不超过j的所有方案组成的集合，设该集合最大能获得最大价值为$f[i][j]$
...
$$
递归时，计算$f[son][j]$后，对于一个孩子结点来说，父结点只能选择某一个体积，因此以孩子节点的体积分组，将其看做分组背包问题，对于父结点来说，所有孩子结点的选择可以看成一个01背包问题
...
| [313. 花店橱窗](https://www.acwing.com/problem/content/315/) |      |
...
| [286. 选课](https://www.acwing.com/problem/content/288/)     |      |
...
| [1072. 树的最长路径](https://www.acwing.com/problem/content/description/1074/) |      |
| [1078. 旅游规划](https://www.acwing.com/problem/content/description/1080/) |      |
...
| [529. 宝藏](https://www.acwing.com/problem/content/531/)     |      |
...
![](C:\Users\Steven\Desktop\jvm\状态机模型.png)
...
![](C:\Users\Steven\Desktop\jvm\状态机2.png)
