...
I/O设备将自己的状态放入状态寄存器，CPU通过不断地查询状态寄存器中的外设的状态，来控制主机与外设数据的交换，直到I/O设备准备就绪，才执行读写

优点：简单、易控制、外围接口控制少

不足：过多占用CPU，效率低，速度慢，查询开销大

#### 中断

需要IO操作时，会启动外设进行IO，然后CPU转去执行其他的工作，执行IO的进程进入阻塞状态，当外设就绪时，向CPU发送中断请求，CPU响应中断请求，调用中断服务程序，执行数据传送，然后返回被打断的程序继续执行

缺点：一次数据交换CPU为了响应和执行中断服务程序需要执行许多额外的操作：包括保存断点、保存现场、设置中断屏蔽字等，对于需要读写频繁的磁盘，如果采用这种方式，导致CPU响应和处理中断的额外开销变大

#### DMA

应用场合：高速设备、数据间间隔时间短的成批数据交换

作用：DMA可以使得CPU在不参与IO的情况下，能够自行将IO数据放入内存中。

工作方式

* CPU对DMA发起指令，告诉需要读取多少数据以及读取到内存哪个地方。发送完命令以后，CPU转去执行其他任务。
* DMA控制器回向磁盘控制器发出指令，磁盘控制器会将数据读取到它的缓冲区，然后会将缓冲区中的数据传输到内存中。
* 当磁盘控制器把数据传输到内存后，磁盘控制器会在总线发去一个确认成功信号给DMA控制器
* DMA控制器收到信号后，DMA控制器会发起DMA完成请求的中断信号，CPU检测到信号后就暂停当前执行的任务，转去执行中断程序
...
* **关于同步、异步、阻塞、非阻塞，在不同的层面下讨论是不同的，这里主要指的是networkIO**
...
![](C:\Users\Steven\Desktop\CS-Note\os\pic\传统文件IO.png)
...
零拷贝目的是为了提高文件的传输效率，它是指将数据直接从磁盘文件复制到网卡设备当中，而不需要经过应用程序。针对如何优化文件传输一般有两种思路，一种是减少上下文切换次数，这个关键是减少系统调用次数，另一种是对于不需要经过应用程序的加工的数据，直接从磁盘传输到网卡，减少数据的拷贝次数。



### mmap+write

使用mmap来代替read，mmap的原理是将内核缓冲区的数据直接映射到对应的用户缓冲区，减少了减少了一次CPU拷贝
...
sendfile替代了read和write两个系统调用，减少了一次系统调用，同时对于传输的数据来说，不需要经过应用程序加工，因此可以不用拷贝到用户态缓冲区当中，而是通过sendfile先将磁盘的数据拷贝到内核缓冲区，再将内核缓冲区拷贝到socket缓冲区当中，最后将socket缓冲区拷贝到网卡的缓冲区当中，这样减少了一次CPU拷贝。
...
如果DMA还支持SG-DMA（The Scatter-Gather Direct Memory Access）技术，那么可以进一步减少socket缓冲区到网卡缓冲区数据的拷贝。

在支持SG-DMA技术下，sendfile系统调用发生了一点改变

* 第一步，将磁盘的数据拷贝到内核缓冲区
* 将缓冲区描述符和数据长度传给socket缓冲区，这样DMA可以根据内存地址以及偏移地址来批量将读缓冲区的数据直接拷贝到网卡，最终实现了真正的零拷贝技术
...
之前所说的内核缓冲区实际上指的是磁盘的高速缓冲区，由于磁盘读取速度必内存慢得多，利用局部性原理，采用了缓存提高了磁盘读取性能。磁盘高速缓冲区主要有以下两个优点

* 缓存了最近被读取的数据
* 磁盘预读取

但是，在大文件传输的情况下，使用PageCache白白浪费了DMA多做的一次数据拷贝，反而会导致磁盘性能下降，因为大文件传输会导致以下两个问题

* 热点的小文件会被大文件占据，小文件无法利用缓存的特性
* 大文件只被读取一次，大文件也没有享受到缓存的好处



**优化**

使用异步IO+直接IO：绕过PageCache，直接将数据从磁盘控制器缓冲区拷贝到用户缓冲区当中
